<template>
  <div class="space-y-6 bg-white border border-gray-200 rounded-3xl p-6 shadow-sm">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-5">
      <div>
        <h2 class="text-xl font-bold text-slate-900 uppercase tracking-tight">{{ title }}</h2>
        <p class="text-xs text-slate-500 mt-1">Please select an active vehicle from the queue to open the workspace.</p>
      </div>
      ...
      <div class="relative w-full sm:w-72">
        <input 
          ...
          class="w-full text-xs bg-slate-50 border border-gray-200 rounded-xl py-2.5 pl-9 pr-4 text-slate-800 focus:bg-white focus:outline-none focus:border-indigo-500 transition-colors"
        />
        <svg class="w-4 h-4 text-slate-450 absolute left-3 top-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
      </div>
    </div>
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center py-20 gap-3">
      ...
      <span class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Syncing queue state...</span>
    </div>

    <!-- Empty State -->
    <div v-else-if="filteredItems.length === 0" class="text-center py-24 border border-dashed border-gray-200 rounded-2xl p-6">
      <div class="w-12 h-12 rounded-full bg-slate-100 text-slate-400 flex items-center justify-center mx-auto mb-3 text-lg">
        ✓
      </div>
      <h3 class="text-xs font-bold text-slate-700 uppercase tracking-widest">No vehicles in queue</h3>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
      <div 
        ...
        :key="item.id"
        class="bg-white border border-gray-200 hover:border-gray-300 rounded-2xl p-4.5 transition flex flex-col justify-between gap-4.5 shadow-sm"
      >
        <div>
          <div class="flex justify-between items-start">
            <span class="text-[9px] font-mono font-bold text-slate-400">#JC-{{ String(item.id).padStart(5, '0') }}</span>
            <span :class="getPriorityClass(item.priority_level)" class="px-2 py-0.5 rounded text-[8px] font-bold uppercase tracking-widest">
              {{ item.priority_level || 'normal' }}
            </span>
          </div>

          <h3 class="font-extrabold text-slate-800 text-sm mt-2">
            {{ item.vehicle?.make || 'Vehicle' }} {{ item.vehicle?.model || '' }}
          </h3>
          <p class="text-[10px] text-indigo-650 font-mono mt-0.5 uppercase">
            Plate: {{ item.vehicle?.license_plate || item.vehicle?.registration_no || 'Pending Plate' }}
          </p>

          <div class="mt-3.5 space-y-1.5 border-t border-gray-150 pt-3.5 text-[10px] text-slate-500">
            <div class="flex justify-between">
              <span>Customer Name:</span>
              <span class="font-bold text-slate-700 truncate max-w-[150px]">{{ item.customer?.name || 'Walk-in Customer' }}</span>
            </div>
            <div v-if="item.workshop_bay?.name" class="flex justify-between">
              <span>Current Bay:</span>
              <span class="font-bold text-emerald-600">{{ item.workshop_bay.name }}</span>
            </div>
            <div v-if="item.mechanic?.name" class="flex justify-between">
              <span>Mechanic:</span>
              <span class="font-bold text-indigo-600">{{ item.mechanic.name }}</span>
            </div>
            <div v-if="item.complaint" class="pt-1.5 text-slate-650 italic leading-snug">
        ...
        <button 
          ...
          class="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-center text-white rounded-xl font-bold text-[10px] uppercase tracking-wider transition-all duration-150 border border-transparent shadow-sm"
        >
