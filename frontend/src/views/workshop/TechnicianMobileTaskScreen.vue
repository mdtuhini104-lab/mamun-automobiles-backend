  ...
  <div class="max-w-md mx-auto space-y-6 p-5 bg-slate-950 border border-slate-900 rounded-3xl shadow-2xl text-slate-100 min-h-[90vh] pb-24 flex flex-col justify-between">
    ...
    <div class="flex items-center justify-between border-b border-slate-900 pb-4">
      ...
      <div class="flex items-center gap-2">
        <span v-if="offlineQueueCount > 0" class="px-2 py-0.5 rounded bg-amber-500/10 border border-amber-500/20 text-amber-400 font-extrabold text-[8px] uppercase tracking-wider font-mono">
          Queue: {{ offlineQueueCount }}
        </span>
        <div class="flex items-center gap-1.5 shrink-0">
          <span :class="isOnline ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.5)]'" class="inline-flex h-2.5 w-2.5 rounded-full transition-all duration-300"></span>
    ...
    <div class="flex-1 space-y-4">
      <div class="flex justify-between items-center mt-4">
        ...
        <button @click="fetchTasks" class="text-[10px] font-black uppercase tracking-wider px-2.5 py-1 bg-slate-900 hover:bg-slate-850 rounded-lg border border-slate-800 text-indigo-400 hover:text-indigo-300 transition duration-200">Sync Tasks</button>
      </div>
      ...
      <div v-else-if="tasks.length === 0" class="text-center py-20 border border-dashed border-slate-900 rounded-2xl p-4 text-slate-650 text-xs italic">
      ...
      <div v-else class="space-y-4">
        <div 
          v-for="task in tasks" 
          :key="task.id"
          class="bg-slate-900/90 border border-slate-800/80 rounded-2xl p-4 space-y-4 flex flex-col justify-between shadow-lg"
        >
          <div>
            ...
            <h4 class="font-bold text-white text-sm mt-1.5">{{ task.task_name }}</h4>
            <p class="text-[10px] text-slate-400 font-mono mt-1">Vehicle: {{ task.work_order?.job_card?.vehicle?.make }} {{ task.work_order?.job_card?.vehicle?.model }} ({{ task.work_order?.job_card?.vehicle?.registration_no }})</p>
            
            <!-- Task Media Section -->
            <div class="mt-3.5 space-y-2 border-t border-slate-950 pt-3">
              ...
              <div v-if="task.photos && task.photos.length > 0" class="flex gap-2 overflow-x-auto py-1">
                <img v-for="(img, idx) in task.photos" :key="idx" :src="img" class="w-11 h-11 rounded-lg object-cover border border-slate-800 shrink-0" />
              </div>
              <div v-else class="text-[9px] text-slate-550 italic">No progress photos uploaded.</div>
              <button 
                type="button" 
                @click="mockTaskPhotoUpload(task)"
                class="px-3 py-1.5 bg-slate-950 hover:bg-slate-900 border border-slate-850 rounded-lg text-[9px] text-indigo-400 font-bold uppercase transition"
              >
                + Upload Photo
              </button>
            </div>
          ...
          <div class="flex gap-2.5 border-t border-slate-950 pt-4">
            <button 
              v-if="task.status === 'pending' || task.status === 'paused'"
              @click="updateStatus(task.id, 'in_progress')"
              class="flex-1 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-200 active:scale-[0.98] shadow-md"
            >
              ▶ Start
            </button>
            <button 
              ...
              class="flex-1 py-3 bg-slate-800 hover:bg-slate-750 text-slate-350 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-200 active:scale-[0.98] border border-slate-700/50"
            >
              ⏸ Pause
            </button>
            <button 
              ...
              class="flex-1 py-3 bg-emerald-600 hover:bg-emerald-750 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-200 active:scale-[0.98] shadow-md"
            >
              ✔ Complete
            </button>
            <button 
              @click="promptConsumeParts(task)"
              class="py-3 px-4.5 bg-slate-800 hover:bg-slate-750 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-200 active:scale-[0.98] border border-slate-700/50"
            >
              + Part
            </button>
          </div>
    ...
    <div v-if="showPartsModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm">
      <div class="bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl max-w-sm w-full p-5 space-y-4 text-slate-100 animate-scaleUp">
        ...
        <form @submit.prevent="submitPartsConsumption" class="space-y-4">
          <div>
            <label class="block text-[10px] text-slate-400 mb-1.5 uppercase font-black">Select Part *</label>
            <select v-model="partsForm.part_id" required class="w-full text-xs bg-slate-950 border border-slate-800 rounded-xl p-3 text-white focus:outline-none focus:border-indigo-500">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-[10px] text-slate-400 mb-1.5 uppercase font-black">Quantity *</label>
              <input 
                v-model.number="partsForm.quantity" 
                type="number" 
                required 
                class="w-full text-xs bg-slate-950 border border-slate-800 rounded-xl p-3 text-white font-mono focus:outline-none focus:border-indigo-500"
              />
            </div>
            <div>
              <label class="block text-[10px] text-slate-400 mb-1.5 uppercase font-black">Ownership *</label>
              <select v-model="partsForm.source_type" class="w-full text-xs bg-slate-950 border border-slate-800 rounded-xl p-3 text-white focus:outline-none focus:border-indigo-500">
                <option value="workshop_supplied">Workshop Supplied</option>
                <option value="customer_supplied">Customer Supplied</option>
              </select>
            </div>
          </div>
          <div class="flex justify-end gap-3 pt-3 border-t border-slate-950">
            <button type="button" @click="showPartsModal = false" class="px-4 py-2.5 border border-slate-800 rounded-xl text-xs font-bold text-slate-350">Cancel</button>
            <button type="submit" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-black uppercase tracking-wider">Log Parts</button>
          </div>
        </form>
      </div>
    </div>
