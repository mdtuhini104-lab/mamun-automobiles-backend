<template>
  <div class="workshop-live-board-container p-6 bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl text-slate-100 min-h-screen">
    <!-- Header with statistics -->
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-8 border-b border-slate-800 pb-6">
      <div>
        ...
        <p class="text-sm text-slate-400 mt-1">Real-time enterprise workflow activation desk. Auto-synced via CAN-Bus and WebSockets.</p>
      </div>

      <!-- Quick stats -->
      <div class="flex flex-wrap items-center gap-3">
        <!-- Connection state -->
        <div class="flex items-center gap-2 px-3 py-2 rounded-xl border bg-slate-950/80 transition-all duration-300 border-indigo-500/30">
          <span class="relative flex h-2 w-2">
            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-indigo-400 opacity-75"></span>
            <span class="relative inline-flex rounded-full h-2 w-2 bg-indigo-500"></span>
          </span>
          <span class="text-[9px] font-black uppercase tracking-widest text-slate-350">
            WS: Connected
          </span>
        </div>
        ...
        <button 
          ...
          class="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-white rounded-xl transition duration-200 border border-slate-700 shadow-md font-bold text-xs flex items-center gap-1.5 focus:outline-none"
        >
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center py-20 gap-3">
      ...
      <p class="text-xs text-slate-450 font-bold uppercase tracking-widest">Rebuilding Board State...</p>
    </div>

    <!-- Horizontal Kanban Columns -->
    <div v-else class="flex overflow-x-auto gap-6 pb-6 scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-slate-950 min-h-[70vh]">
      ...
      >
        ...
        <div class="flex items-center justify-between border-b border-slate-850 pb-3">
          <div class="flex items-center gap-2.5">
            ...
            <h3 class="text-xs font-black uppercase tracking-wider text-slate-350">{{ col.name }}</h3>
          </div>
          <span class="bg-slate-850 text-slate-300 text-[10px] font-black px-2.5 py-0.5 rounded-full border border-slate-800">
            {{ col.items.length }}
          </span>
        </div>
        ...
        <div class="flex-1 overflow-y-auto space-y-3.5 pr-1 scrollbar-thin">
          <div v-if="col.items.length === 0" class="flex flex-col items-center justify-center py-24 text-center border border-dashed border-slate-850/60 rounded-2xl p-4">
            <span class="text-slate-600 text-lg mb-1.5">✓</span>
            <p class="text-[9px] text-slate-600 font-bold uppercase tracking-wider">Empty Stage</p>
          </div>
          ...
          <div 
            ...
            :key="item.id"
            class="bg-slate-900 border border-slate-800/80 hover:border-slate-700/80 rounded-2xl p-4 transition-all flex flex-col justify-between gap-4 shadow-sm"
          >
            <div>
              ...
              <h4 class="font-extrabold text-white text-xs mt-2">
                {{ item.vehicle?.make }} {{ item.vehicle?.model }}
              </h4>
              <p class="text-[9px] text-slate-450 font-mono mt-0.5">
              ...
              <div class="mt-3 space-y-1.5 border-t border-slate-850 pt-3 text-[10px] text-slate-400">
                <div class="flex justify-between">
                  <span>Customer:</span>
                  <span class="font-bold text-slate-350 truncate max-w-[130px]">{{ item.customer?.name || 'Walk-in' }}</span>
                </div>
            ...
            <router-link
              ...
              class="w-full py-2 bg-slate-800 hover:bg-slate-750 text-center text-white rounded-xl font-black text-[9px] uppercase tracking-wider transition-colors border border-slate-700/60"
            >
              {{ col.actionLabel }}
            </router-link>
          </div>
...
const getPriorityClass = (priority) => {
  const map = {
    'low': 'bg-slate-850 text-slate-450 border border-slate-800',
    'normal': 'bg-slate-850 text-slate-450 border border-slate-800',
  ...
  return map[priority?.toLowerCase()] || 'bg-slate-850 text-slate-450';
};
