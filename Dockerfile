...
# Force PHP-FPM to listen on 127.0.0.1:9001 strictly to match nginx, completely replacing zz-docker.conf to avoid 'multiply defined' errors
RUN echo "[global]" > /usr/local/etc/php-fpm.d/zz-docker.conf \
    ...
    && echo "listen = 127.0.0.1:9001" >> /usr/local/etc/php-fpm.d/zz-docker.conf \
